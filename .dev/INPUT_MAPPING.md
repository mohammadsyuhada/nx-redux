...
| flycast (DC.pak) | no swap: flycast reads the pad directly and its face binds are positional (bottom = DC A, right = B, left = X, top = Y) under either layout. `launch.sh` installs the shipped `SDL_Xbox 360 Controller.cfg` unconditionally on every launch, for **both** the Dreamcast mapping and its `_arcade.cfg` sibling — nothing in NX Redux can edit these files (flycast's own controls page is unreachable), so a later change reaches every card on its next launch; `nx_overlay.cpp` reads the env var |
