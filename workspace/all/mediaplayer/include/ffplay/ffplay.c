...
#define MAX_QUEUE_SIZE (64 * 1024 * 1024)
...
#define OSD_TEXT_Y_OFFSET 16
#define OSD_TITLE_HEIGHT 68
#define OSD_TITLE_SCALE 3 /* VGA font scale: 8*3=24px wide, 16*3=48px tall */
#define OSD_TITLE_Y_OFFSET 10

/* D-pad hold repeat state */
static int hat_held_direction = 0; /* SDL_HAT_LEFT/RIGHT/UP/DOWN or 0 */
static int64_t hat_last_seek_time = 0;
#define HAT_REPEAT_DELAY_US 400000	  /* initial delay before repeat */
#define HAT_REPEAT_INTERVAL_US 150000 /* repeat interval */

static void osd_show(void) {
	osd_visible = 1;
	osd_last_activity = av_gettime_relative();
}

/* 5x7 bitmap font for digits 0-9, symbols, and letters */
static const unsigned char font_5x7[][7] = {
	...
	/* : (10) */ {0x00, 0x04, 0x04, 0x00, 0x04, 0x04, 0x00},
	/* / (11) */ {0x01, 0x02, 0x02, 0x04, 0x08, 0x08, 0x10},
	/*   (12) */ {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
	/* - (13) */ {0x00, 0x00, 0x00, 0x0E, 0x00, 0x00, 0x00},
	/* P (14) */ {0x1E, 0x11, 0x11, 0x1E, 0x10, 0x10, 0x10},
	/* A (15) */ {0x0E, 0x11, 0x11, 0x1F, 0x11, 0x11, 0x11},
	/* U (16) */ {0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x0E},
...
static void osd_draw_text(int x, int y, const char* text, int scale,
						  int r, int g, int b, int a) {
	SDL_SetRenderDrawColor(renderer, r, g, b, a);
	while (*text) {
		int idx = font_char_index(*text);
		int row, col;
		for (row = 0; row < 7; row++) {
		...
		x += 6 * scale; /* 5 pixels + 1 gap */
		text++;
	}
}

/* Draw text using FFmpeg's built-in VGA 8x16 font (full ASCII support) */
static void osd_draw_vga_text(int x, int y, const char* text, int scale,
							  int r, int g, int b, int a, int max_w) {
	int start_x = x;
	SDL_SetRenderDrawColor(renderer, r, g, b, a);
	while (*text) {
		unsigned char c = (unsigned char)*text;
		const uint8_t* glyph = &avpriv_vga16_font[c * 16];
		int row, col;
		/* Stop if we'd exceed max width */
		if (max_w > 0 && (x - start_x + 8 * scale) > max_w)
			break;
		...
		x += 8 * scale; /* 8 pixels wide, no extra gap needed */
		text++;
	}
}

/* Extract display title from file path: strip directory and extension */
static void osd_get_title(const char* path, char* out, int outsize) {
	const char *name, *dot;
	if (!path) {
		out[0] = '\0';
		return;
...
static void format_time(char* buf, int bufsize, double secs) {
	int total, h, m, s;
	if (secs < 0)
		secs = 0;
	total = (int)secs;
...
static void osd_draw(VideoState* is) {
	double pos, dur, frac;
	int w, h, bar_x, bar_w, bar_y, fill_w, text_scale;
	int dot_x, dot_r;
	char time_cur[32], time_dur[32], time_str[80];

	/* Keep OSD visible while paused; otherwise auto-hide after timeout */
	if (is->paused) {
		osd_visible = 1;
	} else if (!osd_visible) {
		return;
	} else if (av_gettime_relative() - osd_last_activity > OSD_TIMEOUT_US) {
		osd_visible = 0;
		return;
	}
	...
	{
		char title[256];
		SDL_Rect tbg = {0, 0, w, OSD_TITLE_HEIGHT};
		SDL_SetRenderDrawColor(renderer, 0, 0, 0, 180);
		SDL_RenderFillRect(renderer, &tbg);
		if (window_title && window_title != input_filename)
			snprintf(title, sizeof(title), "%s", window_title);
		else
			osd_get_title(input_filename, title, sizeof(title));
		osd_draw_vga_text(OSD_MARGIN, OSD_TITLE_Y_OFFSET, title,
						  OSD_TITLE_SCALE, 255, 255, 255, 230,
						  w - 2 * OSD_MARGIN);
	}

	/* Bottom bar background */
	{
		SDL_Rect bg = {0, h - OSD_BG_HEIGHT, w, OSD_BG_HEIGHT};
		SDL_SetRenderDrawColor(renderer, 0, 0, 0, 180);
		SDL_RenderFillRect(renderer, &bg);
	}

	/* Time text: "0:23 / 3:45" or "0:23" if no duration */
	text_scale = 4;
	format_time(time_cur, sizeof(time_cur), pos);
	if (dur > 0) {
		format_time(time_dur, sizeof(time_dur), dur);
		snprintf(time_str, sizeof(time_str), "%s / %s", time_cur, time_dur);
	} else {
		snprintf(time_str, sizeof(time_str), "%s", time_cur);
	}

	/* Time text on the left */
	osd_draw_text(OSD_MARGIN, h - OSD_BG_HEIGHT + OSD_TEXT_Y_OFFSET,
				  time_str, text_scale, 255, 255, 255, 230);

	/* "PAUSED" label on the right */
	if (is->paused) {
		int pause_text_w = 6 * 6 * text_scale; /* 6 chars * (5px + 1gap) * scale */
		osd_draw_text(w - OSD_MARGIN - pause_text_w,
	...
	/* Progress bar */
	bar_x = OSD_MARGIN;
	bar_w = w - 2 * OSD_MARGIN;
	bar_y = h - OSD_MARGIN / 2 - OSD_BAR_HEIGHT;

	/* Track */
	{
		SDL_Rect track = {bar_x, bar_y, bar_w, OSD_BAR_HEIGHT};
	...
	if (dur > 0) {
		...
		{
			SDL_Rect fill_rect = {bar_x, bar_y, fill_w, OSD_BAR_HEIGHT};
		...
		/* Playhead dot */
		dot_x = bar_x + fill_w;
		dot_r = OSD_BAR_HEIGHT + 6;
		{
			SDL_Rect dot = {dot_x - dot_r / 2, bar_y - (dot_r - OSD_BAR_HEIGHT) / 2,
							dot_r, dot_r};
...
static void video_refresh(void* opaque, double* remaining_time) {
	...
	display:
		/* display picture */
		if (!display_disable && is->show_mode == SHOW_MODE_VIDEO && is->pictq.rindex_shown && (is->force_refresh || is->paused))
			video_display(is);
	}
	is->force_refresh = 0;
	if (show_status) {
		...
		if (last_w != frame->width || last_h != frame->height || last_format != frame->format || last_serial != is->viddec.pkt_serial || last_vfilter_idx != is->vfilter_idx) {
			...
			if ((ret = configure_video_filters(graph, is, cur_vf, frame)) < 0) {
				...
				if (cur_vf) {
					...
					if (graph) {
						graph->nb_threads = filter_nbthreads;
						ret = configure_video_filters(graph, is, NULL, frame);
					}
				}
				if (ret < 0) {
					SDL_Event event;
...
static void refresh_loop_wait_event(VideoState* is, SDL_Event* event) {
	...
	while (!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT)) {
		...
		if (hat_held_direction != 0) {
			...
			int64_t elapsed = now - hat_last_seek_time;
			int64_t threshold = (hat_last_seek_time == 0) ? HAT_REPEAT_DELAY_US : HAT_REPEAT_INTERVAL_US;
			if (elapsed >= threshold) {
				SDL_Event synth;
				memset(&synth, 0, sizeof(synth));
				synth.type = SDL_JOYHATMOTION;
				synth.jhat.value = hat_held_direction;
				SDL_PeepEvents(&synth, 1, SDL_ADDEVENT, 0, 0);
				hat_last_seek_time = now;
			}
		}
...
static void event_loop(VideoState* cur_stream) {
	...
	/* Open joystick for gamepad input (TrimUI handheld) */
	...
	for (;;) {
		...
		case SDL_JOYBUTTONDOWN:
			...
			case 0: /* B = quit */
				do_exit(cur_stream);
				break;
			case 1: /* A = toggle pause */
				toggle_pause(cur_stream);
				osd_show();
				break;
			case 4: /* L1 = seek back 60s */
				osd_show();
				incr = -60.0;
				goto do_seek;
			case 5: /* R1 = seek forward 60s */
				osd_show();
				incr = 60.0;
				goto do_seek;
		...
		case SDL_JOYHATMOTION:
			...
			case SDL_HAT_LEFT:
				osd_show();
				hat_held_direction = SDL_HAT_LEFT;
				incr = -10.0;
				goto do_seek;
			case SDL_HAT_RIGHT:
				osd_show();
				hat_held_direction = SDL_HAT_RIGHT;
				incr = 10.0;
				goto do_seek;
			case SDL_HAT_UP: {
				double pos = get_clock(&cur_stream->vidclk);
...
int main(int argc, char** argv) {
	...
	osd_show(); /* Show OSD briefly at playback start */
	event_loop(is);
