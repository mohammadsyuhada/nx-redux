...
// Shared minarch state and types, used by the ma_* modules.
// Split from the former minarch.c monolith: types and globals that cross
// module boundaries live here; everything else stays file-static in its module.
...
// Screenshot writer thread (defined in ma_menu.c, joined at shutdown in minarch.c)
extern SDL_Thread* screenshotsavethread;
...
// Button mapping tables and label arrays (defined in ma_config.c / ma_input.c)
...
extern const char* device_button_names[];
...
/* -----------------------------------------------------------------------
   Cross-module functions that live in minarch.c
   ----------------------------------------------------------------------- */
...
// HDMI hotplug monitor (defined in minarch.c)
void hdmimon(void);
