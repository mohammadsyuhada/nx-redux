#!/usr/bin/env bash
# PORTMASTER: steelassault.zip, SteelAssault.sh
...
if [ -d "/opt/system/Tools/PortMaster/" ]; then
  ...
  controlfolder="/mnt/SDCARD/Emus/shared/PortMaster"
fi
...
# Setup savedir (bind mount instead of symlink — exFAT has no symlink support)
mkdir -p "$GAMEDIR/savedata"
$ESUDO umount ~/.local/share/SteelAssault 2>/dev/null || true
$ESUDO umount ~/.config/SteelAssault 2>/dev/null || true
mkdir -p ~/.local/share/SteelAssault
mkdir -p ~/.config/SteelAssault
$ESUDO mount -o bind "$GAMEDIR/savedata" ~/.local/share/SteelAssault
$ESUDO mount -o bind "$GAMEDIR/savedata" ~/.config/SteelAssault
